...
let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  if (!/image\/(jpe?g|png)/i.test(mime)) {
    ...
    return m.reply(`${getBotEmoji(global.mePn)} Envía una imagen o responde a una imagen para convertir a estilo manga.`)
  }
  ...
  try {
    ...
    if (!data.success || !data.result) {
      await conn.sendMessage(m.chat, { react: { text: '❌', key: m.key } })
      return m.reply(`${getBotEmoji(global.mePn)} Error al convertir la imagen a estilo manga.`)
    }
...
// Función auxiliar para obtener buffer del mensaje
...
handler.help = ['manga']
handler.tags = ['tools', 'image', 'anime']
